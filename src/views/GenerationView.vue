<template>
  <div class="page-margin max-w-4xl mx-auto px-4 py-8">
    <button @click="$router.push('/')" class="mb-4 text-sm ink-body hover:underline" style="color: var(--ink-light)">{{ isEn ? '← Back to Home' : '← 返回首頁' }}</button>

    <h1 class="ink-title text-3xl font-bold text-center mb-2 tracking-widest">{{ isEn ? `Generation ${gen}` : `第${gen}世` }}</h1>
    ...
    <p class="text-center mb-8 text-sm" style="color: var(--ink-light)">
      {{ isEn ? `${genMembers.length} members` : `共 ${genMembers.length} 人` }}
      <span v-if="generationChars[gen] && generationChars[gen] !== '—'"> · {{ isEn ? `Generation character "${generationChars[gen]}"` : `輩分字「${generationChars[gen]}」` }}</span>
    </p>
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-baseline gap-2 flex-wrap">
              ...
              <span v-if="m.courtesy" class="text-sm" style="color: var(--ink-light)">{{ isEn ? 'Courtesy: ' : '字' }}{{ m.courtesy }}</span>
              <span v-if="m.alias" class="text-sm" style="color: var(--ink-light)">{{ isEn ? 'Art Name: ' : '號' }}{{ m.alias }}</span>
              <span v-if="m.posthumous" class="text-sm" style="color: var(--red-seal)">{{ isEn ? 'Posthumous: ' : '謚' }}{{ m.posthumous }}</span>
              <span v-if="m.note === '女'" class="text-xs px-1.5 py-0.5 rounded" style="background: rgba(212,87,74,0.1); color: var(--red-marriage)">{{ isEn ? 'F' : '女' }}</span>
            </div>
            <div v-if="m.birth || m.death" class="text-xs mt-1" style="color: var(--ink-medium)">
              <span v-if="m.birth">{{ isEn ? 'Born ' : '生 ' }}{{ m.birth }}</span>
              <span v-if="m.birthNote" class="ml-1" style="color: var(--ink-faint)">（{{ m.birthNote }}）</span>
              <span v-if="m.death"> · {{ isEn ? 'Died ' : '卒 ' }}{{ m.death }}</span>
            </div>
            ...
            <div v-if="m.spouses?.length" class="text-xs mt-1" style="color: var(--red-marriage)">
              {{ isEn ? 'Spouse: ' : '配偶：' }}{{ m.spouses.map(s => s.name).join('、') }}
            </div>
      ...
      <div v-if="!genMembers.length" class="text-center py-12 ink-body" style="color: var(--ink-faint)">
        {{ isEn ? 'No records for this generation' : '該世代暫無人員記錄' }}
      </div>
    </div>
